...
public class Fatura implements Comparable<Object> {
	 ...
	 * 
	 * @param empregado
	 *            the empregado to initialize
	 * @param cliente
	 *            the cliente to initialize
	 * @param restaurante
	 *            the restaurante to initialize
	 * @param mesa
	 *            the mesa to initialize
	 * @param data
	 *            the data to initialize
	 * @param numero
	 *            the numero to initialize
	 **********************************************************************/
	public Fatura(Empregado empregado, Cliente cliente,
			Restaurante restaurante, Mesa mesa, CalendarDate data,
			Integer numero) {
	...
	public void checkRestauranteNaoPodeTerMaisQue10ProdutosDevolvidos() {
		// (Pedido.allInstances->collect($e : Pedido |
		// $e.devolvido)->count(true) < 100)
		int produtosDevolvidos = 0;
		for (Pedido p : Pedido.allInstances()) {
			if (p.devolvido())
				produtosDevolvidos++;
		}
		boolean invariant = produtosDevolvidos <= 10;
	...
	public void checkCozinheirosNaoFaturam() {
		...
		boolean invariant = empregado.tipo() != TipoEmpregado.Cozinheiro;
	...
	/**********************************************************************
	 * @param other
	 *            Fatura to compare to the current one
	 * @return 0 if the argument is equal to the current Fatura; a value less
	 *         than 0 if the argument is greater than the current Fatura; and a
	 *         value greater than 0 if the argument is less than this Fatura.
	 **********************************************************************/
	@Override
	...
	/**********************************************************************
	 * @param other
	 *            Fatura to check equality to the current one
	 * @return true if the argument is equal to the current Fatura and false
	 *         otherwise
	 **********************************************************************/
	@Override
	public boolean equals(Object other) {
		...
		if (!super.equals(another))
			return false;
		if ((this.data == null) ? (another.data != null) : !this.data
				.equals(another.data))
			return false;
		if ((this.numero == null) ? (another.numero != null) : !this.numero
				.equals(another.numero))
			return false;
	...
	public String toString() {
		return "Fatura[data=" + data() + ", numero=" + numero() + ", cliente("
				+ (cliente() == null ? "0" : "1") + "), empregado("
				+ (empregado() == null ? "0" : "1") + "), mesa("
				+ (mesa() == null ? "0" : "1") + "), pedidos("
				+ pedidos().size() + "), restaurante("
				+ (restaurante() == null ? "0" : "1") + ")]";
	}
