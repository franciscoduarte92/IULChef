...
public class Empregado extends Entidade implements Comparable<Object>
{
	...
	* @param idade the idade to initialize
	* @param tipo the tipo to initialize
	**********************************************************************/
	public Empregado(String morada, Integer nc, String nome, Integer idade, TipoEmpregado tipo)
	{
	...
	* @param quantidade the quantidade to set
	* @param quantidades_ingredientes the quantidades_ingredientes to set
	...
	**********************************************************************/
	...
	{
		//	TODO conclude the implementation for this SOIL specification:
		//	declare quantidade : Integer, contagem : Contagem, produtos_simples : Set(Produto), invent : Inventario; invent := new Inventario; invent.init(data); produtos_simples := self.empregadores->collect($e : Restaurante | $e.produtos)->asSet; for ps in produtos_simples do quantidade := produtos_simples->select(p : Produto | (p.nome = ps.nome))->collect($e : Produto | $e.quantidade)->sum; contagem := new Contagem; contagem.existencias := quantidade; insert (contagem,ps.oclAsType(ProdutoSimples)) into Contagem_ProdutoSimples; insert (invent,contagem) into Inventario_Contagem end
	}
	...
	**********************************************************************/
	public void PassaFatura(Cliente c, Mesa m, CalendarDate data, Integer numF)
	{
		...
		//	-----------------------------------------------------------------------------
		//	TODO conclude the implementation for this SOIL specification:
		//	declare f : Fatura; f := new Fatura; f.init(numF, data); insert (f,c) into Fatura_Cliente; insert (f,self) into Fatura_Empregado; insert (f,m) into Fatura_Mesa
	}
	...
	**********************************************************************/
	...
	/**********************************************************************
	* @param other Empregado to compare to the current one
	* @return 0 if the argument is equal to the current Empregado;
	* a value less than 0 if the argument is greater than the current Empregado;
	* and a value greater than 0 if the argument is less than this Empregado.
	**********************************************************************/
	@Override
	...
	/**********************************************************************
	* @param other Empregado to check equality to the current one
	* @return true if the argument is equal to the current Empregado and false otherwise
	**********************************************************************/
	@Override
	public boolean equals(Object other)
	{
		...
		if (!super.equals(another))
			return false;
		if ((this.idade == null) ? (another.idade != null) : !this.idade.equals(another.idade))
			return false;
	...
	**********************************************************************/
	...
	{
		return "Empregado[idade="+idade() + ", tipo="+tipo() + ", morada="+morada() + ", nc="+nc() + ", nome="+nome() + ", contrato(" + contrato().size() + "), empregadores(" + empregadores().size() + "), faturas(" + faturas().size() + ")]";
	}
