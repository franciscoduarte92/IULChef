...
public class Empregado extends Entidade implements Comparable<Object> {
	...
	public void FazerInventario(CalendarDate data) {
		...
		// Set(Produto), invent : Inventario; invent := new Inventario;
		// invent.init(data); produtos_simples := self.empregadores->collect($e
		// : Restaurante | $e.produtos)->asSet; for ps in produtos_simples do
		// quantidade := produtos_simples->select(p : Produto | (p.nome =
		// ps.nome))->collect($e : Produto | $e.quantidade)->sum; contagem :=
		// new Contagem; contagem.existencias := quantidade; insert
