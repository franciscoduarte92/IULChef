...
public class ProdutoComposto extends Produto implements Comparable<Object> {
	 ...
	 * 
	 * @param restaurantes
	 *            the restaurantes to initialize (inherited)
	 * @param nome
	 *            the nome to initialize (inherited)
	 * @param precoVenda
	 *            the precoVenda to initialize (inherited)
	 * @param quantidade
	 *            the quantidade to initialize (inherited)
	 * @param tipo
	 *            the tipo to initialize (inherited)
	 * @param unidade
	 *            the unidade to initialize (inherited)
	 * @param grupo
	 *            the grupo to initialize
	 **********************************************************************/
	public ProdutoComposto(Set<Restaurante> restaurantes, String nome,
			Double precoVenda, Integer quantidade, TipoProduto tipo,
			Unidade unidade, GrupoProdutoComposto grupo) {
	...
	public void checkProdutoCompostoTemNoMinimoTresIngredientes() {
		...
		boolean invariant = ingredientes().size() >= 3;
	...
	public void checkProdutoCompostoTemQueSerConfecionado() {
		...
		boolean invariant = confecionados().size() > 0;
	...
	/**********************************************************************
	 * @param other
	 *            ProdutoComposto to compare to the current one
	 * @return 0 if the argument is equal to the current ProdutoComposto; a
	 *         value less than 0 if the argument is greater than the current
	 *         ProdutoComposto; and a value greater than 0 if the argument is
	 *         less than this ProdutoComposto.
	 **********************************************************************/
	@Override
	public int compareTo(Object other) {
		...
		// return this.precoVenda.compareTo(((ProdutoComposto)
		// other).precoVenda);
		// return this.quantidade.compareTo(((ProdutoComposto)
		// other).quantidade);
	...
	/**********************************************************************
	 * @param other
	 *            ProdutoComposto to check equality to the current one
	 * @return true if the argument is equal to the current ProdutoComposto and
	 *         false otherwise
	 **********************************************************************/
	@Override
	public boolean equals(Object other) {
		...
		if (!super.equals(another))
			return false;
		if ((this.grupo == null) ? (another.grupo != null) : !this.grupo
				.equals(another.grupo))
			return false;
	...
	public String toString() {
		return "ProdutoComposto[grupo=" + grupo() + ", nome=" + nome()
				+ ", precoVenda=" + precoVenda() + ", quantidade="
				+ quantidade() + ", tipo=" + tipo() + ", unidade=" + unidade()
				+ ", composicao(" + composicao().size() + "), confecionados("
				+ confecionados().size() + "), ingredientes("
				+ ingredientes().size() + "), pedidos(" + pedidos().size()
